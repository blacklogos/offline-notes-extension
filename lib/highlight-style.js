...
/**
 * The runs between two offsets of a highlight's text.
 * A highlight is often painted as several <mark> elements, one per block or
 * inline span it crosses, and each needs only the part of the runs it covers.
 */
function sliceRuns(runs, from, to) {
  ...
  for (const run of runs) {
    ...
    out.push({
      text: run.text.slice(Math.max(0, from - start), Math.min(run.text.length, to - start)),
      strong: run.strong,
    });
  }
  return out;
...
/**
 * Fill an element with a highlight's text, wrapping emphasised runs in
 * <strong>. Shared so the live page and the reader render identically.
 */
function paintRuns(el, runs) {
  ...
  for (const part of runs) {
    if (part.strong && part.text.trim()) {
      ...
      frag.appendChild(el.ownerDocument.createTextNode(part.text));
    }
  }
  el.textContent = '';
  el.appendChild(frag);
...
if (typeof window !== 'undefined') {
  window.HighlightStyle = { HIGHLIGHT_COLORS, DEFAULT_HIGHLIGHT_COLOR, colorOf, emphasisRuns, sliceRuns, paintRuns, quoteToMarkdown, addEmphasis };
}
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { HIGHLIGHT_COLORS, DEFAULT_HIGHLIGHT_COLOR, colorOf, emphasisRuns, sliceRuns, paintRuns, quoteToMarkdown, addEmphasis };
}
