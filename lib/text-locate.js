...
(() => {
  ...
  function locateNormalized(haystack, quote, context) {
    ...
    // No cap on collection: a quote repeated many times is exactly the case
    // where the stored context is needed, and stopping early threw away the
    // occurrence that context would have identified. The scan is linear and
    // bounded by the article length.
    const hits = [];
    let from = 0;
    for (;;) {
  ...
  // Offset translation between two renderings whose only difference is
  // whitespace presence rather than whitespace amount.
  function translateTight(source, target, start, end) {
    const a = tightenWithMap(source);
    const b = tightenWithMap(target);
    if (!a.tight || a.tight !== b.tight) return null;

    const srcToTight = new Array(source.length);
    for (let i = 0; i < a.map.length; i++) srcToTight[a.map[i]] = i;

    let ts = -1;
    for (let i = start; i < source.length; i++) { if (srcToTight[i] !== undefined) { ts = srcToTight[i]; break; } }
    let te = -1;
    for (let i = end - 1; i >= 0; i--) { if (srcToTight[i] !== undefined) { te = srcToTight[i]; break; } }
    if (ts < 0 || te < 0 || te < ts) return null;

    const s0 = b.map[ts];
    const e0 = b.map[te];
    if (s0 === undefined || e0 === undefined) return null;
    return { start: s0, end: e0 + 1 };
  }
  ...
  function translateRange(source, target, start, end) {
    ...
    if (a.norm !== b.norm || !a.norm) {
      // The two are not the same words under normalisation, which happens
      // when the quote was located by the whitespace-insensitive fallback: a
      // selection dragged across blocks joins them with no separator while
      // the article keeps a blank line. Compare with all whitespace removed,
      // which is the same condition that matched them in the first place.
      return translateTight(source, target, start, end);
    }
